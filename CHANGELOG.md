...
- **Analytics value normalization** — `value_map` context type enables automatic consolidation of variant field values during `fmrug_analyze` groupby. Mappings saved via `fmrug_save_context` persist in FM and apply immediately.
- **Publish fork workflow** — Automated scrubbed public releases with version sync, scrub verification, GitHub release creation, and back-link tracking.

### Changed

- **`fmrug_save_context` tool description** — Documents `value_map` context type, JSON format requirement, and auto-apply behavior so the LLM correctly formats mapping requests.
- **Project structure** — Scripts reorganized into `release/`, `diagnostics/`, `business/` subdirs. Docs split into `public/`, `internal/` subdirs.
